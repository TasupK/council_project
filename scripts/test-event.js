...
function installCommonStubs_(context) {
  context.normalizeEventText_ = function (value) { return value === null || typeof value === 'undefined' ? '' : String(value).trim(); };
  context.requireEventRequestId_ = function (request) { return String((request && (request.id || request.eventId || request.applicationId)) || '').trim(); };
  ...
  context.paginateEventItems_ = function (items, request) {
    ...
    return { items: items.slice(start, start + pageSize), page: page, pageSize: pageSize, total: items.length };
  };
}
...
function createQueryContext_() {
  ...
  context.listEventPaymentClientRows_ = function () { return []; };
...
function testEventData_() {
  ...
  context.findEventRowById_ = function (id) { return id === 'event-1' ? { id: 'event-1', name: '행사', __rowNumber: 2 } : null; };
  context.throwEventError_ = function (code, message) { var error = new Error(message); error.code = code; throw error; };
  assert.deepStrictEqual(JSON.parse(JSON.stringify(context.getEventForEditData_({ id: 'event-1' }))), { id: 'event-1', name: '행사' });
  assert.throws(function () { context.getEventForEditData_({ id: 'missing' }); }, function (error) { return error.code === 'NOT_FOUND'; });
}

function testEventList_() {
  var context = createQueryContext_();
  var rows = [
    { id: 'event-1', name: '개강 행사', managerEmail: 'manager-a', category: '복지', status: '모집', eventStartAt: '2026-09-02' },
    { id: 'event-2', name: '종강 행사', managerEmail: 'manager-b', category: '문화', status: '종료', eventStartAt: '2026-06-20' },
    { id: 'event-3', name: '체육 행사', managerEmail: 'manager-a', category: '체육', status: '예정', eventStartAt: '2026-10-01' }
  ];
  context.listEventClientRows_ = function () { return rows.map(function (row) { return Object.assign({}, row); }); };
  var result = context.getEventListData_({ filter: { managerEmail: 'manager-a', includeClosed: false }, page: 1, pageSize: 10 });
  assert.deepStrictEqual(result.items.map(function (item) { return item.id; }), ['event-3', 'event-1']);
  assert.deepStrictEqual(JSON.parse(JSON.stringify(result.summary)), { total: 3, scheduled: 1, recruiting: 1, inProgress: 0, closed: 1 });
  assert.deepStrictEqual(JSON.parse(JSON.stringify(result.options)), {
    managers: ['manager-a', 'manager-b'], eventTypes: ['문화', '복지', '체육'], eventStatuses: ['예정', '모집', '진행', '종료']
  });
}
...
function testApplicantListAndDetail_() {
  ...
  context.findEventApplicationRowById_ = function () { return { id: 'app-1', eventId: 'event-1', name: '김학생', appliedFee: 1000, __rowNumber: 2 }; };
  context.findEventAttendanceRowByApplicationId_ = function () { return { id: 'attendance-1', applicationId: 'app-1', status: '출석', __rowNumber: 4 }; };
  context.listEventPaymentClientRows_ = function () { return [{ applicationId: 'app-1', paidAmount: 1000 }, { applicationId: 'app-2', paidAmount: 500 }]; };
...
function testAttendanceList_() {
  ...
  context.listEventAttendanceClientRows_ = function () {
    return [{ applicationId: 'app-1', confirmedAt: '2026-08-01T10:00:00', status: '출석', managerEmail: 'staff@example.com' }];
  };
  context.listEventApplicationClientRows_ = function () {
    return [
  ...
  assert.deepStrictEqual(JSON.parse(JSON.stringify(result.items)), [{
    applicationId: 'app-1', studentId: '6001', name: '김학생', phone: '010-1111', applicantType: '재학생', appliedFee: 1000,
    paidAmount: 1000, confirmedAt: '2026-08-01T10:00:00', status: '출석', managerEmail: 'staff@example.com'
  }]);
}
