...
function testUsersSettingsComposition_() {
  ...
  context.getUserDbFields_ = function () {
    return {
      email: 'Google이메일',
      name: '성명',
      studentId: '학번',
  ...
  context.listUserRows_ = function () {
    return [{
      'Google이메일': 'Student@Example.com ',
      '성명': '김학생',
      '학번': '6001',
      '연락처': '010-1111-2222',
      '계정상태': '활성',
      '최종수정일시': '2026-08-17T12:00:00',
      '등록자이메일': 'admin@example.com'
    }];
  };
  context.getRolesById_ = function () {
    return { ROLE_ADMIN: { id: 'ROLE_ADMIN', name: '관리자' } };
  };
  context.getActiveRoleIdsByEmail_ = function () {
    return { 'student@example.com': ['ROLE_ADMIN'] };
  };
...
function testRolesSettingsComposition_() {
  ...
  installValueStubs_(context);
  context.getUserDbFields_ = function () {
    return { roleId: '역할ID', assignedStatus: '배정상태' };
  };
  load_(context, 'src/000_server/030_auth/roles.gs');
  context.getRolesById_ = function () {
    return {
      ROLE_ADMIN: { id: 'ROLE_ADMIN', assignedCount: 0 },
      ROLE_STAFF: { id: 'ROLE_STAFF', assignedCount: 0 }
    };
  };
  context.listUserRoleRows_ = function () {
    return [
      { '역할ID': 'ROLE_ADMIN', '배정상태': '활성' },
      { '역할ID': 'ROLE_ADMIN', '배정상태': '활성' },
      { '역할ID': 'ROLE_ADMIN', '배정상태': '비활성' },
      { '역할ID': 'ROLE_STAFF', '배정상태': '활성' }
    ];
  };
...
function testPermissionsSettingsComposition_() {
  ...
  context.getUserDbFields_ = function () {
    return {
      id: '권한ID', area: '업무영역', action: '행위', name: '권한명',
      description: '권한설명', active: '활성여부'
    };
  };
  ...
  context.getPermissionsById_ = function () {
    return {
      EVENT_VIEW: context.toPermissionDto_(rows[0]),
      EVENT_EDIT: context.toPermissionDto_(rows[1])
    };
  };
  context.getPermissionIdsByRoleId_ = function () {
    return { ROLE_ADMIN: ['EVENT_VIEW', 'EVENT_EDIT', 'MISSING'] };
  };
