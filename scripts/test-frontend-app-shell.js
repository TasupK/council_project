...
assert.match(header, /id=["']appSidebarToggle["']/); assert.match(header, /aria-controls=["']appSidebar["']/); assert.match(sidebar, /id=["']appSidebar["']/);
assert.match(shellStyles, /\.app\s*\{[\s\S]*height:\s*100vh/); assert.match(shellStyles, /grid-template-rows:\s*var\(--header-h\)\s+minmax\(0,\s*1fr\)/); assert.match(shellStyles, /\.header\s*\{/); assert.match(shellStyles, /\.sidebar\s*\{[\s\S]*overflow-y:\s*auto/); assert.match(shellStyles, /\.main\s*\{[\s\S]*overflow:\s*auto/);
['.app','.header','.body','.sidebar','.main','.nav-item','.nav-group','.nav-submenu','.nav-subitem','.global-search','.term-select','.icon-btn','.user-trigger','.pop'].forEach(selector => { const escaped=selector.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'); assert.doesNotMatch(appStyles,new RegExp('(^|\\})\\s*'+escaped+'\\s*\\{','m')); });
assert.match(shellJs,/localStorage/); assert.match(shellJs,/sidebar-hidden/); assert.match(shellJs,/aria-expanded/); assert.match(shellJs,/aria-hidden/);
const templates=[
...
templates.forEach(file=>{ const source=read(file); assert.doesNotMatch(source,/<footer\b[^>]*status-bar/); assert.match(source,/include\(['"]frontend\/shared\/styles\/App_Styles['"]\)/); assert.match(source,/include\(['"]frontend\/widgets\/app_header\/App_Header['"]\)/); assert.match(source,/include\(['"]frontend\/widgets\/app_sidebar\/App_Sidebar['"]\)/); assert.match(source,/include\(['"]frontend\/app\/styles\/App_Shell_Styles['"]\)/); assert.match(source,/include\(['"]frontend\/app\/shell\/app_shell_js['"]\)/); });
