...
required.forEach(function (rel) {
  assert.ok(fs.existsSync(path.join(root, rel)), 'missing Event Home FSD file: ' + rel);
});
assert.ok(!fs.existsSync(path.join(root, 'src/600_event/610_home')), 'legacy Event Home slice must be removed');
...
[
  ...
  "include('frontend/features/event_list/event_list_js')",
  "include('frontend/pages/event_home/event_home_controller_js')"
].forEach(function (needle) { assert.ok(shell.includes(needle), 'Event Home shell missing include: ' + needle); });
...
assert.ok(!shell.includes('600_event/610_home/'));
assert.ok(view.includes('id="ew-event-search"'));
assert.ok(view.includes('id="ew-event-table"'));
assert.ok(controller.includes('initializeEventList()'));
['eventClient.getEvents', 'eventClient.closeEvent', 'eventClient.updateEventStatus'].forEach(function (needle) {
  assert.ok(feature.includes(needle), 'Event list feature missing semantic client call: ' + needle);
});
assert.ok(!feature.includes('google.script.run'));
assert.ok(!/['"]api_[A-Za-z0-9_]+['"]/.test(feature));
assert.ok(feature.includes('function initializeEventList'));
assert.ok(router.includes("event: 'frontend/pages/event_home/Event_Home'"));
