...
function createContext() {
  ...
  load(context, 'src/backend/app/config/config.gs');
  return context;
}

(function testFeeApplicationSchemaContract() {
  const context = createContext();
  load(context, 'src/backend/core/db/schema/operation_db_schema.gs');
...
(function testCoveragePolicy() {
  const context = createContext();
  load(context, 'src/backend/domains/student_fee/business_rules/student_fee_coverage_policy.gs');

  function coverage(input) {
    return JSON.parse(JSON.stringify(context.calculateStudentFeeCoverage_(input)));
  }

  assert.deepStrictEqual(coverage({ currentSemesterId: '20261', academicYearLevel: 1, semesterWithinYear: 1, coverageMode: 'STANDARD_REMAINING' }), { startSemesterId: '20261', semesterCount: 8 });
  assert.strictEqual(coverage({ currentSemesterId: '20271', academicYearLevel: 2, semesterWithinYear: 1, coverageMode: 'STANDARD_REMAINING' }).semesterCount, 6);
  assert.strictEqual(coverage({ currentSemesterId: '20284', academicYearLevel: 4, semesterWithinYear: 2, coverageMode: 'STANDARD_REMAINING' }).semesterCount, 1);
  assert.strictEqual(coverage({ currentSemesterId: '20261', academicYearLevel: 1, semesterWithinYear: 1, coverageMode: 'BROAD_FIRST_YEAR' }).semesterCount, 2);
  assert.strictEqual(coverage({ currentSemesterId: '20271', academicYearLevel: 2, semesterWithinYear: 1, coverageMode: 'BROAD_AFTER_ASSIGNMENT' }).semesterCount, 6);
  assert.throws(() => context.calculateStudentFeeCoverage_({ currentSemesterId: '20261', academicYearLevel: 0, semesterWithinYear: 1, coverageMode: 'STANDARD_REMAINING' }), /academicYearLevel/);
  assert.throws(() => context.calculateStudentFeeCoverage_({ currentSemesterId: '20261', academicYearLevel: 1, semesterWithinYear: 3, coverageMode: 'STANDARD_REMAINING' }), /semesterWithinYear/);
  assert.throws(() => context.calculateStudentFeeCoverage_({ currentSemesterId: '20261', academicYearLevel: 2, semesterWithinYear: 1, coverageMode: 'BROAD_FIRST_YEAR' }), /1학년/);
  assert.throws(() => context.calculateStudentFeeCoverage_({ currentSemesterId: '20261', academicYearLevel: 1, semesterWithinYear: 1, coverageMode: 'UNKNOWN' }), /coverageMode/);
})();
...
(function testApprovalUsesFullCoverageAmount() {
  ...
  load(context, 'src/backend/domains/student_fee/application/fee_payments_mutation.gs');
...
(function testApprovalRejectsInvalidCoverageCount() {
  [undefined, '', 0, 1.5, 9].forEach((semesterCount) => {
    ...
    context.upsertFeePayerFromApplication_ = () => null;
    load(context, 'src/backend/domains/student_fee/application/fee_payments_mutation.gs');
    assert.throws(() => context.processFeeApplicationsData_({ ids: ['app-invalid'], action: 'APPROVE' }, { email: 'staff@example.com' }), /적용학기수|semesterCount/);
  });
})();

function fakeItemResponse(title, response) {
  return { getItem: () => ({ getTitle: () => title }), getResponse: () => response };
}
...
function fakeFormResponse(overrides) {
  const values = Object.assign({
    ...
    items: [
      ...
      fakeItemResponse('납입날짜', '2026-08-19'), fakeItemResponse('현재학년', '2학년'), fakeItemResponse('현재학기', '1학기'),
      fakeItemResponse('납부유형', '일반 납부'), fakeItemResponse('학생카드캡쳐', ['https://drive.google.com/open?id=STUDENT_CARD_FILE']),
      fakeItemResponse('입금내역캡쳐', ['https://drive.google.com/file/d/DEPOSIT_FILE/view'])
    ]
  }, overrides || {});
  return { getId: () => values.id, getTimestamp: () => values.timestamp, getItemResponses: () => values.items };
}

(function testFormMapperNormalizesAuthoritativeResponse() {
  const context = createContext();
  load(context, 'src/backend/domains/student_fee/repositories/fee_form_mapper.gs');
...
(function testFormMapperCoverageModes() {
  const context = createContext();
  load(context, 'src/backend/domains/student_fee/repositories/fee_form_mapper.gs');
  const broadFirst = fakeFormResponse({ id: 'BROAD-FIRST', items: [fakeItemResponse('학번', '20261002'), fakeItemResponse('성명', '광역학생'), fakeItemResponse('소속', '경영대학'), fakeItemResponse('납입날짜', '2026-08-19'), fakeItemResponse('현재학년', '1학년'), fakeItemResponse('현재학기', '1학기'), fakeItemResponse('납부유형', '광역 1학년 납부')] });
  assert.strictEqual(context.mapStudentFeeFormResponse_(broadFirst).coverageMode, 'BROAD_FIRST_YEAR');
  const broadAfter = fakeFormResponse({ id: 'BROAD-AFTER', items: [fakeItemResponse('학번', '20261002'), fakeItemResponse('성명', '광역학생'), fakeItemResponse('소속', '경영정보학과'), fakeItemResponse('납입날짜', '2027-03-02'), fakeItemResponse('현재학년', '2학년'), fakeItemResponse('현재학기', '1학기'), fakeItemResponse('납부유형', '광역 학과확정 추가납부')] });
...
(function testFormMapperRejectsMissingRequiredData() {
  ...
  assert.throws(() => context.mapStudentFeeFormResponse_(fakeFormResponse({ items: [fakeItemResponse('성명', '이름만 있음')] })), /학번/);
  assert.throws(() => context.mapStudentFeeFormResponse_(fakeFormResponse({ items: [fakeItemResponse('학번', '20261001'), fakeItemResponse('성명', '김학생'), fakeItemResponse('소속', '경영정보학과'), fakeItemResponse('납입날짜', '2026-08-19'), fakeItemResponse('현재학년', '5학년'), fakeItemResponse('현재학기', '1학기'), fakeItemResponse('납부유형', '일반 납부')] })), /현재학년/);
})();
...
(function testFormReaderIsReadOnly() {
  ...
  let openedId = '';
  context.FormApp = { openById: (formId) => { openedId = formId; return { getResponses: () => responses }; } };
  load(context, 'src/backend/domains/student_fee/repositories/fee_form_reader.gs');
