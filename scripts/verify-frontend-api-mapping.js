...
function collectStaticApiReferences(source, frontendPath) {
  ...
  var result = [];
  var match;
  while ((match = pattern.exec(cleaned)) !== null) result.push({ name: match[0], path: frontendPath, line: lineNumber(cleaned, match.index) });
  return result;
}
...
function extractFunctionBodies(source) {
  ...
  while ((match = pattern.exec(source)) !== null) {
    ...
    if (close < 0) continue;
    result.push({ name: match[1], params: match[2].trim() ? match[2].split(',').map(function (value) { return value.trim(); }) : [], start: match.index, end: close + 1, body: source.slice(open + 1, close) });
...
function collectGasDispatchers(source, frontendPath) {
  ...
  return extractFunctionBodies(cleaned).reduce(function (items, fn) {
    ...
    fn.params.forEach(function (param) {
      ...
      if (new RegExp('\\[\\s*' + escaped + '\\s*\\]').test(fn.body)) items.push({ name: fn.name, param: param, path: frontendPath, start: fn.start, end: fn.end });
    });
    return items;
  }, []);
}
...
function collectDirectDynamicCalls(source, frontendPath, dispatcherRanges) {
  var cleaned = stripCommentsPreserveLines(source);
  var result = [];
  [/google\.script\.run[\s\S]{0,500}?\[\s*([A-Za-z_$][\w$]*)\s*\]/g, /\brunner\s*\[\s*([A-Za-z_$][\w$]*)\s*\]/g].forEach(function (pattern) {
    var match;
    while ((match = pattern.exec(cleaned)) !== null) {
      var insideDispatcher = dispatcherRanges.some(function (range) { return range.path === frontendPath && match.index >= range.start && match.index < range.end; });
      if (!insideDispatcher) result.push({ expression: match[1], path: frontendPath, line: lineNumber(cleaned, match.index), via: 'direct' });
    }
  });
  return result;
}
...
function auditSources(frontendSources, serverSources) {
  var serverApis = [];
  serverSources.forEach(function (item) { serverApis = serverApis.concat(collectServerApis(item.source, item.path)); });
  var serverNames = {};
  serverApis.forEach(function (api) { serverNames[api.name] = api; });
  var dispatchers = [];
  frontendSources.forEach(function (item) { dispatchers = dispatchers.concat(collectGasDispatchers(item.source, item.path)); });
  var references = [];
  var dynamicCalls = [];
  ...
  return { serverApis: serverApis, frontendReferences: references, missingServerApis: missingServerApis, dynamicCalls: dynamicCalls, dynamicDispatchers: dispatchers, unusedServerApis: unusedServerApis };
}

function auditSourcePair(frontendSource, serverSource, options) {
  options = options || {};
  return auditSources([{ source: frontendSource, path: options.frontendPath || 'frontend.html' }], [{ source: serverSource, path: options.serverPath || 'server.gs' }]);
}

function auditRepository(root) {
  var srcRoot = path.join(root, 'src');
  var backendRoots = [path.join(srcRoot, 'backend'), path.join(srcRoot, '000_server')].filter(function (candidate) { return fs.existsSync(candidate); });
  var frontendFiles = listFiles(srcRoot, function (file) {
    return !backendRoots.some(function (backendRoot) { return isInsideRoot(file, backendRoot); }) && /\.(html|js)$/.test(file);
  });
  var serverFiles = [];
  backendRoots.forEach(function (backendRoot) { serverFiles = serverFiles.concat(listFiles(backendRoot, function (file) { return /\.(gs|js)$/.test(file); })); });
  var frontendSources = frontendFiles.map(function (file) { return { source: fs.readFileSync(file, 'utf8'), path: path.relative(root, file).replace(/\\/g, '/') }; });
  var serverSources = serverFiles.map(function (file) { return { source: fs.readFileSync(file, 'utf8'), path: path.relative(root, file).replace(/\\/g, '/') }; });
  return auditSources(frontendSources, serverSources);
}

function formatAuditFailures(result) {
  var lines = [];
  result.missingServerApis.forEach(function (item) { lines.push('Missing server API: ' + item.name + ' referenced at ' + item.path + ':' + item.line); });
  result.dynamicCalls.forEach(function (item) { lines.push('Unresolved dynamic API dispatch: ' + item.expression + ' at ' + item.path + ':' + item.line); });
  return lines.join('\n');
}

module.exports = { auditSourcePair: auditSourcePair, auditRepository: auditRepository, formatAuditFailures: formatAuditFailures, collectServerApis: collectServerApis, stripCommentsPreserveLines: stripCommentsPreserveLines };

if (require.main === module) {
  var result = auditRepository(path.resolve(__dirname, '..'));
  var failures = formatAuditFailures(result);
  if (failures) { console.error(failures); process.exitCode = 1; }
  else {
    ...
    result.unusedServerApis.forEach(function (api) { console.log('  - ' + api.name + ' (' + api.path + ':' + api.line + ')'); });
  }
}
