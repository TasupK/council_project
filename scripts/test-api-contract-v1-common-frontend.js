...
assert.ok(!/callMyPageApi_/.test(mypageController));
assert.ok(!/google\.script\.run/.test(mypageController));
assert.ok(!/google\.script\.run/.test(notificationFeature));
...
templates.forEach(function (relativePath) {
  ...
  assert.ok(html.indexOf("include('frontend/entities/user/api/app_client_js')") >= 0, relativePath + ' must include migrated app client');
});
