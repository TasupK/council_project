...
var reader = path.join(EVENT, 'repositories/applicant_form_reader.gs');
var mapper = path.join(EVENT, 'repositories/applicant_form_mapper.gs');
...
if (!failures.length) {
  ...
  forbid_(serviceSource, /\bFormApp\b|\bSpreadsheetApp\b/, 'Sync application must delegate external reads to repository');
  ...
  if (!/function\s+applyApplicantFormSyncData_\s*\(/.test(serviceSource)) failures.push('Sync orchestration must live in application/applicant_form_sync.gs');
...
if (failures.length) {
  ...
  console.log('Event Form sync migrated architecture verification passed.');
}
