...
assert.ok(!/DB 필드 없음/.test(detailSource), 'department missing-field placeholder must be removed');
assert.ok(!/info\s*\(\s*['\"]학과['\"]/.test(detailSource), 'department row must be excluded from applicant modal');
assert.ok(/data-process=["']approve["']/.test(detailSource), 'approve button must exist');
assert.ok(/data-process=["']reject["']/.test(detailSource), 'reject button must exist');
assert.ok(/처리 담당자/.test(detailSource) && /processedAt/.test(detailSource), 'processor details must be displayed');
