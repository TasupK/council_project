...
var h = makeContext();
h.context.createLedgerEntryData_({ transaction_type: '지출 ', amount: 1000 }, { email: 'staff@example.com' }, 'ACTIVE');
...
['기타', '', 'expense'].forEach(function (type) {
  var c = makeContext();
  assert.throws(function () { c.context.createLedgerEntryData_({ transaction_type: type, amount: 1000 }, {}, 'ACTIVE'); }, /거래유형|transaction_type/);
});

[0, -1, 'abc', Infinity].forEach(function (amount) {
  var c = makeContext();
  assert.throws(function () { c.context.createLedgerEntryData_({ transaction_type: '수입', amount: amount }, {}, 'ACTIVE'); }, /금액|amount/);
});
