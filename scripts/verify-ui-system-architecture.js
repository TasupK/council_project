...
['--ui-bg','--ui-surface','--ui-border','--ui-border-strong','--ui-text','--ui-muted','--ui-primary','--ui-primary-hover','--ui-info-bg','--ui-info-fg','--ui-success-bg','--ui-success-fg','--ui-warning-bg','--ui-warning-fg','--ui-danger-bg','--ui-danger-fg','--ui-neutral-bg','--ui-neutral-fg','--ui-radius-sm','--ui-radius-md','--ui-radius-lg','--ui-shadow','--ui-space-1','--ui-space-2','--ui-space-3','--ui-space-4','--ui-space-5','--ui-space-6','--ui-space-8'].forEach(function (token) { if (appStyles.indexOf(token + ':') === -1) fail_('Missing canonical UI token: ' + token); });
['--bg','--surface','--border','--border-strong','--text','--text-2','--text-3','--text-4','--primary','--primary-hover','--active-bg','--active-text','--danger','--green','--radius','--shadow'].forEach(function (token) { var value = getCssVarValue_(appStyles, token); if (value && value.indexOf('var(--ui-') === -1) fail_('Legacy token must alias canonical UI token: ' + token + ' -> ' + value); });
['.app','.header','.body','.sidebar','.main','.sidebar-toggle','.nav-item','.nav-group','.nav-submenu','.nav-subitem','.global-search','.term-select','.icon-btn','.user-trigger','.pop'].forEach(function (selector) { if (hasSelectorBlock_(appStyles, selector)) fail_('Shell selector must not be owned by App_Styles: ' + selector); if (!hasSelectorBlock_(shellStyles, selector)) fail_('Shell selector missing from App_Shell_Styles: ' + selector); });
['.active','.brand','.crumb','.page','.side','.top','.shell'].forEach(function (selector) { if (hasSelectorBlock_(appStyles, selector) || hasSelectorBlock_(shellStyles, selector)) fail_('Unscoped shared selector is forbidden: ' + selector); });
[['.ui-btn'],['.ui-field'],['.ui-card'],['.ui-stat-card'],['.ui-badge'],['.ui-table-wrap'],['table.ui-table'],['.ui-modal'],['.ui-tabs'],['.ui-tab'],['.ui-pagination'],['.ui-toast'],['.ui-loading','.ui-empty'],['.ui-error']].forEach(function (selectors) { selectors.forEach(function (selector) { if (!hasSelectorBlock_(appStyles, selector) && appStyles.indexOf(selector + ',') === -1) fail_('Missing shared UI primitive: ' + selector); }); });
['.ui-btn.primary','.ui-btn.outline','.ui-btn.ghost','.ui-btn.danger','.ui-btn.small','.ui-btn.large','.ui-badge.success','.ui-badge.warning','.ui-badge.danger','.ui-badge.info','.ui-badge.neutral'].forEach(function (selector) { if (appStyles.indexOf(selector) === -1) fail_('Missing shared UI modifier: ' + selector); });
...
var SHELL_TEMPLATES = [
  ...
  'src/400_accounting/400_home/Accounting_Home.html','src/frontend/pages/accounting_ledger/Accounting_Ledger.html','src/frontend/pages/accounting_reconciliation/Accounting_Reconciliation.html','src/frontend/pages/accounting_settlement/Accounting_Settlement.html',
