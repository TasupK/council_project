...
const runnerPath = path.join(root, 'src/frontend/shared/api/rpc/app_api_runner_js.html');
...
(async function () {
  ...
  templates.forEach(function(relativePath){ const html=fs.readFileSync(path.join(root,relativePath),'utf8'); assert.ok(html.indexOf("include('frontend/shared/api/rpc/app_api_runner_js')")>=0, relativePath+' must include the shared RPC runner'); });
