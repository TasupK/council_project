...
function makeGas(mode, value, received) {
  ...
  return { script: { run: new Proxy(runner, { get: function (target, prop) {
    if (prop in target) return target[prop];
    return function (payload) {
      received.push({ name: String(prop), payload: payload });
      if (mode === 'success') target._success(value); else target._failure(value);
    };
  } }) } };
}
...
(async function () {
  let received = [];
  let context = loadRunner(makeGas('success', { ok: true, data: { value: 1 } }, received));
  let data = await context.runAppApi('api_ping', { id: 'A' });
  assert.deepStrictEqual(JSON.parse(JSON.stringify(data)), { value: 1 });
  assert.deepStrictEqual(JSON.parse(JSON.stringify(received)), [{ name: 'api_ping', payload: { request: { id: 'A' } } }]);

  received = [];
  context = loadRunner(makeGas('success', { ok: true, data: null }, received));
  data = await context.runAppApi('api_ping');
  assert.strictEqual(data, null);
  assert.deepStrictEqual(JSON.parse(JSON.stringify(received[0].payload)), { request: {} });

  received = [];
  context = loadRunner(makeGas('success', { value: 1 }, received));
  await assert.rejects(context.runAppApi('api_ping', {}), error => error && error.code === 'INVALID_API_RESPONSE');

  const transported = new Error('__APP_ERROR__:' + JSON.stringify({ code: 'NOT_FOUND', message: '행사를 찾을 수 없습니다.', details: { id: 'A' } }));
  received = [];
  context = loadRunner(makeGas('failure', transported, received));
  await assert.rejects(context.runAppApi('api_ping', {}), error => error && error.code === 'NOT_FOUND' && error.message === '행사를 찾을 수 없습니다.' && error.details.id === 'A');

  const wrappedTransported = new Error('Error: __APP_ERROR__:' + JSON.stringify({ code: 'FORBIDDEN', message: '요청한 업무 권한을 찾을 수 없습니다.', details: {} }));
  received = [];
  ...
  templates.forEach(function (relativePath) {
    ...
    const expected = migrated.has(relativePath) ? "include('frontend/shared/api/app_api_runner_js')" : "include('100_common/app_api_runner_js')";
