...
function makeGas(mode, value, received) {
  ...
  return {
    script: {
      run: new Proxy(runner, {
        get: function (target, prop) {
          if (prop in target) return target[prop];
          return function (payload) {
            received.push({ name: String(prop), payload: payload });
            if (mode === 'success') target._success(value);
            else target._failure(value);
          };
        }
      })
    }
...
(async function () {
  ...
  assert.deepStrictEqual(JSON.parse(JSON.stringify(received)), [
    { name: 'api_ping', payload: { request: { id: 'A' } } }
  ]);
  ...
  await assert.rejects(
    ...
    function (error) { return error && error.code === 'INVALID_API_RESPONSE'; }
  );

  const transported = new Error('__APP_ERROR__:' + JSON.stringify({
    code: 'NOT_FOUND',
  ...
  await assert.rejects(
    context.runAppApi('api_ping', {}),
    function (error) {
      return error && error.code === 'NOT_FOUND' && error.message === '행사를 찾을 수 없습니다.' && error.details.id === 'A';
    }
  );
