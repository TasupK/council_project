...
function requireIds_(source, ids, label) {
  ids.forEach(function (id) {
    if (source.indexOf('id="' + id + '"') < 0) failures.push(label + ' missing id=' + id);
  });
}

function requireNames_(source, names, label) {
  names.forEach(function (name) {
    if (source.indexOf('name="' + name + '"') < 0) failures.push(label + ' missing name=' + name);
  });
}

function requireActions_(source, actions, label) {
  actions.forEach(function (action) {
    if (source.indexOf('data-action="' + action + '"') < 0) failures.push(label + ' missing action=' + action);
  });
}

var payerModal = readOptional_('510_payers/modals/Student_Fee_Payer_Edit_Modal.html');
requireIds_(payerModal, [
  'sf-payer-modal', 'sf-payer-modal-title', 'sf-payer-modal-desc', 'sf-payer-form',
  'sf-payer-student-id', 'sf-payer-name', 'sf-payer-affiliation-input', 'sf-payer-semester', 'sf-payer-submit'
], 'Payer modal');
...
if (paymentConfirm && (paymentConfirm.indexOf('data-result="MISMATCH"') < 0 || paymentConfirm.indexOf('data-result="DONE"') < 0)) {
  failures.push('Payment confirm modal must preserve MISMATCH and DONE results.');
}
...
requireIds_(refundApproval, [
  'sf-refund-approval-modal', 'sf-refund-approval-title', 'sf-refund-approval-form', 'sf-refund-maximum',
  'sf-refund-approved-amount', 'sf-refund-approval-reason', 'sf-refund-approve-submit'
...
requireIds_(refundTransfer, [
  'sf-refund-transfer-modal', 'sf-refund-transfer-title', 'sf-refund-transfer-form',
  'sf-refund-transfer-date', 'sf-refund-transfer-file', 'sf-refund-transfer-reason'
...
if (refundTransfer && (refundTransfer.indexOf('data-result="FAILED"') < 0 || refundTransfer.indexOf('data-result="DONE"') < 0)) {
  failures.push('Refund transfer modal must preserve FAILED and DONE results.');
}
