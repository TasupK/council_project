...
function readOptional_(relativePath) { var target = path.join(FRONTEND_ROOT, relativePath); return fs.existsSync(target) ? fs.readFileSync(target, 'utf8') : ''; }

var VIEW_FILES = [
  { label: 'frontend/pages/student_fee_home/Student_Fee_Home_View.html', source: readRoot_('src/frontend/pages/student_fee_home/Student_Fee_Home_View.html') },
  { label: '510_payers/Student_Fee_Payers_View.html', source: read_('510_payers/Student_Fee_Payers_View.html') },
  { label: '520_payments/Student_Fee_Payments_View.html', source: read_('520_payments/Student_Fee_Payments_View.html') },
  { label: '530_refunds/Student_Fee_Refunds_View.html', source: read_('530_refunds/Student_Fee_Refunds_View.html') }
];

VIEW_FILES.forEach(function (view) {
  var file = view.label;
...
['510_payers/Student_Fee_Payers_View.html','520_payments/Student_Fee_Payments_View.html','530_refunds/Student_Fee_Refunds_View.html'].forEach(function (file) {
  var source = read_(file);
  ['ui-card', 'ui-toolbar', 'ui-table-wrap', 'ui-btn'].forEach(function (primitive) { if (source.indexOf(primitive) < 0) failures.push(file + ' must use shared primitive: ' + primitive); });
});

var MODAL_PAGES = [
  { shell: '510_payers/Student_Fee_Payers.html', view: '510_payers/Student_Fee_Payers_View.html', partials: ['510_payers/modals/Student_Fee_Payer_Edit_Modal.html'] },
  { shell: '520_payments/Student_Fee_Payments.html', view: '520_payments/Student_Fee_Payments_View.html', partials: ['520_payments/modals/Student_Fee_Payment_Detail_Modal.html','520_payments/modals/Student_Fee_Payment_Confirm_Modal.html'] },
  { shell: '530_refunds/Student_Fee_Refunds.html', view: '530_refunds/Student_Fee_Refunds_View.html', partials: ['530_refunds/modals/Student_Fee_Refund_Detail_Modal.html','530_refunds/modals/Student_Fee_Refund_Approval_Modal.html','530_refunds/modals/Student_Fee_Refund_Transfer_Modal.html'] }
];
MODAL_PAGES.forEach(function (page) {
  var shell = read_(page.shell); var view = read_(page.view);
  ...
  page.partials.forEach(function (partial) {
    ...
    if (shell.indexOf("include('500_student_fee/" + includePath + "')") < 0) failures.push(page.shell + ' must include ' + partial + '.');
    if (source) {
...
var homeJs = readRoot_('src/frontend/pages/student_fee_home/student_fee_home_controller_js.html');
