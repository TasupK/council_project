...
function createEventData_(request, context) {
  ...
  payload.managerEmail = String(context && context.userEmail || readActiveUserEmailFromSession_() || '').trim();
...
function updateEventData_(request, context) {
  ...
  patch.managerEmail = String(context && context.userEmail || readActiveUserEmailFromSession_() || '').trim();
